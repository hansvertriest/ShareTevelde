...
const NewPost = ({children}) => {
	const { getCourses, getAssignments, createNewCourse, createNewAssignment, uploadPictureWithFilename,createPost, sendNotification } = useApi();
	...
	const uploadPost = async (ev) => {
		...
		if (assignmentId && pictures.length > 0) {
			await createPost(assignmentId, (url !== 'www.') ? url : '', pictures)
				.then((res) =>{
					sendNotification({
						userId: currentUser.id,
						content: 'Jouw post is gepubliceerd!',
						destinationUrl: `/post/${res._id}`,
						type: 'info',
					});
